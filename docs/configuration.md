...
pixy loads a YAML config (via [`yaconfiglib`](https://github.com/jose-pr/yaconfiglib),
so `!include` and deep-merge are available). By default it reads `pixy.yaml` from
`./config`; override with `--config FILE` or `--baseconfig DIR`.

The top-level keys map to the `Pixy` engine's collections:
...
- **Targets** normalise `ip`/`mac`/`hostname` at load time. If a field is
  missing, pixy fills it in where it can (a MAC-shaped id becomes the `mac`; an
...
`templates` is a list of search paths (local or URI). For each render pixy looks
